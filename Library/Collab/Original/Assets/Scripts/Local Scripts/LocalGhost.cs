...
public class LocalGhost : MonoBehaviour
{
    ...
    [SerializeField] private float prevSpeed; // serialise helps to mark these fields so unity can save and load these values
    ...
    public int scatterModeTime1 = 7;
    public int chaseModeTime1 = 20;
    public int scatterModeTime2 = 5;
    public int chaseModeTime2 = 20;
    ...
    public int pinkyStartTime = 5;
    public int inkyStartTime = 12; // 14?
    public int clydeStartTime = 20; // 21?
    ...
    void DetectCollision()
    {
        ...
        if (ghostRect.Overlaps(pacmanRect))
        {
            ...
            else
            {
                ...
                if (currMode != Mode.Consumed)
                {
                    gameManager.GetComponent<LocalGameManager>().StartDeath(); // call from game manager
                }
            }
    ...
    void UpdateMode()
    {
        ...
        else if (currMode == Mode.Frightened)
        {
            ...
            if (frightTime >= frightenedModeTime)
            {
                ...
                backgroundMusic.clip = gameManager.GetComponent<LocalGameManager>().normalMusic;
    ...
    public void InitiateFrightenedMode()
    {
        ...
        if (currMode != Mode.Consumed)
        {
            LocalGameManager.ghostConsumedScore = 200; // resets to standard 200 points
            // while changing audio, need to tell unity to play
            backgroundMusic.clip = gameManager.GetComponent<LocalGameManager>().powerMusic;
            backgroundMusic.Play();
    ...
    public void InitiateConsumedMode()
    {
        ...
        LocalGameManager.playerScore += LocalGameManager.ghostConsumedScore;
        currMode = Mode.Consumed;
        prevSpeed = speed;
        speed = consumedSpeed;
        UpdateGhostAnimation(); // so that the eyes immediately update instead of only when move is activated
        gameManager.GetComponent<LocalGameManager>().StartConsume(this.GetComponent<LocalGhost>()); // start the consume animation
        LocalGameManager.ghostConsumedScore *= 2; // multiply by 2 if more ghosts are eaten during consumed mode
    }
    ...
    Vector2 GetPinkyTarget()
    {
        ...
        Vector2 targetPos = pacman.transform.localPosition;
        Vector2 targetOrientation = pacman.GetComponent<LocalPacMan>().orientation;
    ...
    Vector2 GetInkyTarget()
    {
        ...
        Vector2 blinkyPos = GameObject.Find("blinky").transform.localPosition;
    ...
    {
        GameObject tile = gameManager.GetComponent<LocalGameManager>().board[(int)pos.x, (int)pos.y];
