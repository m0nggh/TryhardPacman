...
public class LocalPacMan : MonoBehaviour
{
    ...
    void DestroyPellet()
    {
        ...
        if (obj != null)
        {
            ...
            if (tile != null && !tile.isConsumed && (tile.isPellet || tile.isPowerPellet))
            {
                ...
                gameManager.GetComponent<LocalGameManager>().pelletsConsumed++; // consume one pellet
                ...
                {
                    LocalGameManager.playerScore += 100;
                    GameObject[] ghosts = GameObject.FindGameObjectsWithTag("GhostDemon");

                    foreach (GameObject ghost in ghosts)
                    {
                        ghost.GetComponent<LocalGhost>().InitiateFrightenedMode();
                    }
                }
                else
                {
                    LocalGameManager.playerScore += 10;
                }
            }
        }
